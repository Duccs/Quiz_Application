        ...
        public async Task<IActionResult> Login([FromForm] LoginViewModel objCollection)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    if (string.IsNullOrEmpty(value))
                    {
                        IQueryable<Services.Entities.Candidate> candidate = await _candidate.SearchCandidate(x => x.Candidate_ID.Equals(objCollection.Candidate_ID) && x.Password.Equals(objCollection.Password.EncodeBase64()));
                        if (candidate.Any())
                        {
